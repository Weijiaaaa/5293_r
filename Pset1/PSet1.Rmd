...
I think the default method of base r is better because it can give a better bin that can fit different data, as not all data need 30 bins which is determined by the ggplot2. This difference can be shown clearly from the two plots above as well, from the ggplot2 plot we can see there are a lot of gaps between the bars, which means that there is no data in the range.
...
My strategy is that since the biggest difference for the two plots is to include different boundary values, then setting the boundaries of the bars to be decimal values could let non of them include the boundary values which can make the two plots to be identical.
